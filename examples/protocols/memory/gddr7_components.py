...
from jitx_protocols_ext.protocols.memory.gddr7 import GDDR7
...
class SplitBGABase(
    ...
    def _generate_layout(self) -> Iterable[GridPosition]:
        ...
        half_offset = ((self._half_cols - 1) * self._h_pitch + self._gap) / 2.0
        ...
        for r in range(self._num_rows):
            ...
            # First half (columns 0-6)
            for c in range(self._half_cols):
                x = (c - half_center_col) * self._h_pitch - half_offset
                tx = Transform.translate(x, row_y)
                yield GridPosition(r, c, tx)

            # Second half (columns 7-13)
            for c in range(self._half_cols):
                x = (c - half_center_col) * self._h_pitch + half_offset
