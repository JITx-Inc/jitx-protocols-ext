...
from ....protocols.memory.ddr4 import DDR4, DDR4Width, DDR4Rank
...
class SplitBGABase(
    ...
    def _generate_layout(self) -> Iterable[GridPosition]:
        ...
        l = ((self._half_cols - 1) * self._pitch + self._gap) / 2.0
        ...
        for r in range(self._num_rows):
            ...
            # First half (columns 0-2 in grid, map to 1-3 in naming) - offset by -l
            for c in range(self._half_cols):
                x = (c - half_center_col) * self._pitch - l
                tx = Transform.translate(x, row_y)
                yield GridPosition(r, c, tx)

            # Second half (columns 6-8 in grid, map to 7-9 in naming) - offset by +l
            for c in range(self._half_cols):
                x = (c - half_center_col) * self._pitch + l
