...
class GDDR7Constraint(SignalConstraint["GDDR7"]):
    ...
    def constrain(self, src: GDDR7, dst: GDDR7):
        ...
        for ch_idx, (src_ch, dst_ch) in enumerate(zip(src.data, dst.data, strict=True)):
            ...
            # CA signals
            ca_topos = [Topology(src_ca, dst_ca) for src_ca, dst_ca in zip(src_ch.CA, dst_ch.CA, strict=True)]
