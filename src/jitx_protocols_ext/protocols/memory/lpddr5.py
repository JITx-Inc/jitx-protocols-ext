...
class LPDDR5Constraint(SignalConstraint["LPDDR5"]):
    """Signal Integrity Constraint for LPDDR5 (structure-free).
    ...
    src/dst pair. Does **not** bind any
    :py:class:`RoutingStructure` / :py:class:`DifferentialRoutingStructure`
    — trace width / pair spacing / clearance are expected to come from
    tag-based design rules generated by
    :py:func:`lpddr_constraints.make_lpddr_routing_rules` and
    :py:func:`lpddr_constraints.make_lpddr_spacing_rules`.

    Args:
        width: Channel width
        rank: Rank configuration
        params: Constraint parameters (skew + loss). Use defaults derived
            from AMD UG863 unless your channel needs different values.
    """
    ...
    def __init__(
        ...
        # No routing-structure binding — DiffPairConstraint with
        # structure=None enforces just skew + loss on each pair.
    ...
    def constrain(self, src: LPDDR5, dst: LPDDR5):
        ...
        for ch_idx in range(num_ch):
            ...
            # No `.structure(...)` binding — width / spacing come from
            # tag-based design rules generated by lpddr_constraints.
            for rank_idx in range(nr):
                ...
                self.add(Constrain(cs_topo).insertion_loss(self.params.loss))
            ...
            for ca_bit in range(7):
                ...
                self.add(Constrain(ca_topo).insertion_loss(self.params.loss))
            ...
            for lane_idx in range(2):
                ...
                # plus per-topology insertion-loss limit. No structure
                # binding (handled by tag-based design rules).
                for src_dq, dst_dq in zip(src_lane.dq, dst_lane.dq, strict=True):
                    ...
                    self.add(Constrain(dq_topo).insertion_loss(self.params.loss))
                ...
                self.add(Constrain(dmi_topo).insertion_loss(self.params.loss))

        # Reset signal (shared across all channels) — loss only.
        reset_topo = Topology(src.reset_n, dst.reset_n)
        self.add(Constrain(reset_topo).insertion_loss(self.params.loss))
